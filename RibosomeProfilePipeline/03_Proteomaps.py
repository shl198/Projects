...
#===============================================================================
#                         1. generate count data for day3 and day6 proteomaps
#===============================================================================
"""
...
for f,total in zip(posFiles,totalCount):
    geneIDs = []
    handle = open(f,'r')
    counts = []
    for line in handle:
        ...
        rpkm = sum(cov[up-15:-down-14])/total*(10**6)#/len(cov)*(10**9)
        counts.append(rpkm)        
    gene_count_df[f[:3]] = pd.Series(counts)
gene_count_df.insert(0,'GeneID',geneIDs)
gene_count_df.to_csv('/data/shangzhong/RibosomeProfiling/cho_pr/12_pr_gene_rpm.txt',sep='\t',index=False)
"""
...
day3_df.to_csv('/data/shangzhong/RibosomeProfiling/cho_pr/13_day3_rpm.csv',sep='\t',index=False,header=None)
day6_df.to_csv('/data/shangzhong/RibosomeProfiling/cho_pr/13_day6_rpm.csv',sep='\t',index=False,header=None)
print n
"""
#===============================================================================
...
#===============================================================================
#                         3. check percentage of mapping genes in proteomaps
#===============================================================================
"""
...
#===============================================================================
#                         4. get how much percent of gene mapping reads are representaed by proteomaps 
#===============================================================================
...
gene_raw_count_df = pd.read_csv(gene_rawCount_file,sep='\t',header=0)
antibody_df = gene_raw_count_df[gene_raw_count_df['GeneID'].isin(['heavychain','lightchain','NeoRKanR'])]
total = gene_raw_count_df.iloc[:,1:].sum().tolist()
cri = gene_raw_count_df['GeneID'].map(lambda x: mappableID(x,cho_mus_dict,mouseMapIDs))
mappable_df = gene_raw_count_df[cri].append(antibody_df)
count_sum = mappable_df.iloc[:,1:].sum().tolist()
